...
public class ItemCatServiceImpl implements ItemCatService {
    ...
    public void delete(Long[] ids) {
        ...
        //查询子类,子类的子类
        for (Object id : longs) {
            List<TbItemCat> childList =findByParentId((Long) id);
            if (childList!=null){
                for (TbItemCat child : childList) {
                    List<TbItemCat> grandsonList =findByParentId(child.getId());
                    if (grandsonList!=null){
                        for (TbItemCat grandson : grandsonList) {
                            //删除孙类
                            itemCatMapper.delete(grandson);
                        }
                    }
                    //删除子类
                    itemCatMapper.delete(child);
                }
            }
            //删除本身
